...
router.get('/:id', async (req, res) => {
    ...
    SELECT u.UserID, u.FullName AS CandidateName, u.EmailAddress, u.ProfileInfo, u.ProfilePicture,
...
router.post('/', async (req, res) => {
    try {
        const { ElectionName, StartDateTime, EndDateTime, Status, Rules } = req.body;
        
        const [maxId] = await pool.query('SELECT IFNULL(MAX(ElectionID), 0) + 1 AS NewID FROM Election');
        const newElectionId = maxId[0].NewID;
        
        await pool.query(
            'INSERT INTO Election (ElectionID, ElectionName, StartDateTime, EndDateTime, Status, Rules) VALUES (?, ?, ?, ?, ?, ?)',
            [newElectionId, ElectionName, StartDateTime, EndDateTime, Status || 'Upcoming', Rules || '']
        );
        
        res.status(201).json({ success: true, message: 'Election created successfully', electionId: newElectionId });
    } catch (error) {
        console.error(error);
        res.status(500).json({ success: false, error: error.message });
    }
